 ...
 * HTTP boundary. It is **extended as modules adopt** the subsystem — decks are the
 * first (and, in phase-04, only) adopter. The runtime resolver registry is keyed
 * by arbitrary string, so tests can exercise the polymorphic code path with a
 * subject type that is not (yet) in this enum.
 */
export const subjectTypeSchema = z.enum(["deck"]);
